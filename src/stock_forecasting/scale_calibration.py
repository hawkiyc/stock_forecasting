...
def resolve_scale_feature_statistics(
    dataset: LazyFinancialWindowDataset, *, sample_count: int, seed: int,
    ...
    identity = {
        ...
        "split": "train", "sample_count": count, "seed": seed,
        "sampler": "blockwise-16-v1",
    }
    namespace = (
        dataset.root.parent.parent if dataset.root.parent.name == "prepared"
    ...
    if path.is_file():
        ...
        print(json.dumps({
            "scale_feature_calibration": "cache_hit", "sample_count": count,
            "statistics_sha256": cached["sha256"],
        }), flush=True)
        return cached
    print(json.dumps({
        "scale_feature_calibration": "running", "sample_count": count,
    ...
    loader = DataLoader(
        HistoricalScaleDataset(dataset), batch_size=min(256, count),
        sampler=sorted(sampler), pin_memory=False, **loader_options,
    )
    # At most 50k x 8 float64 values in production, independent of full dataset size.
    values = np.empty((count, 8), dtype=np.float64)
    offset = 0
    for batch in loader:
        current = batch["scale_targets"].numpy()
    ...
    print(json.dumps({
        "scale_feature_calibration": "complete", "sample_count": count,
