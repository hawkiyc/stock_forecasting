...
RUNPOD_TPEX_PROXY_SECRET_NAME="${RUNPOD_TPEX_PROXY_SECRET_NAME:-}"
...
if [[ -n "${RUNPOD_CPU_PREPARE_RESERVE_SECONDS}" ]]; then
    if [[ ! "${RUNPOD_CPU_PREPARE_RESERVE_SECONDS}" =~ ^[1-9][0-9]*$ \
        || "${RUNPOD_CPU_PREPARE_RESERVE_SECONDS}" -ge "${RUNPOD_CPU_MAX_RUNTIME_SECONDS}" ]]; then
        echo "RUNPOD_CPU_PREPARE_RESERVE_SECONDS must be positive and shorter than max runtime" >&2
        exit 2
    fi
fi
...
if [[ "${RUNPOD_STAGE}" == "stage1" \
    && ( "${FIN_TS_DATASET_PROFILE}" == "tw_only" \
...
TPEX_PROXY_TOKEN_REFERENCE=""
TPEX_PROXY_POD_URL=""
if [[ "${RUNPOD_STAGE}" == "stage1" \
    && ( "${FIN_TS_DATASET_PROFILE}" == "tw_only" \
    || "${FIN_TS_DATASET_PROFILE}" == "us_tw_eodhd" \
    || "${FIN_TS_DATASET_PROFILE}" == "us_tw_massive" ) ]]; then
    TPEX_PROXY_TOKEN_REFERENCE="{{ RUNPOD_SECRET_${RUNPOD_TPEX_PROXY_SECRET_NAME} }}"
    TPEX_PROXY_POD_URL="${TPEX_PROXY_URL}"
fi
POD_ENV_JSON="$(printf \
    '{"NETWORK_VOLUME_ROOT":"%s","RUNPOD_VOLUME_ROOT":"%s","RUNPOD_EXPECTED_VOLUME_ID":"%s","RUNPOD_REQUESTED_CPU_COUNT":"%s","PROJECT_ROOT":"%s","DATA_ROOT":"%s","RUNPOD_CONFIG":"%s","RUNPOD_STAGE":"%s","RUNPOD_STAGE_CONFIG_SHA256":"%s","RUNPOD_SELECTION_ID":"%s","RUNPOD_SELECTION_SHA256":"%s","RUNPOD_DATASET_REQUEST_SHA256":"%s","RUNPOD_DATASET_REVISION":"%s","RUNPOD_REMOTE_SELECTION_PATH":"%s","RUNPOD_ROLE":"cpu-prep","RUNPOD_CPU_MAX_RUNTIME_SECONDS":"%s","RUNPOD_CPU_PREPARE_RESERVE_SECONDS":"%s","RUNPOD_CPU_MAX_API_CALLS":"%s","RUNPOD_CPU_EODHD_QPS":"%s","RUNPOD_CPU_TAIWAN_QPS":"%s","RUNPOD_PROVIDER_MAX_BACKOFF_SECONDS":"%s","RUNPOD_SHUTDOWN_ACTION":"terminate","RUNPOD_IMAGE":"%s","RUNPOD_EXPECTED_TORCH_VERSION":"%s","RUNPOD_EXPECTED_CUDA_PREFIX":"%s","RUNPOD_EXPECTED_UBUNTU_VERSION":"%s","HF_TOKEN":"%s","EODHD_API_TOKEN":"%s","TPEX_PROXY_URL":"%s","TPEX_PROXY_TOKEN":"%s","FIN_TS_DATASET_PROFILE":"%s","FIN_TS_H_START":"%s","STAGE1_US_SYMBOLS":"%s","STAGE1_US_ETF_SYMBOLS":"%s","STAGE1_SYMBOL_LIMIT":"%s","STAGE1_DATA_START":"%s","STAGE1_DATA_END":"%s"}' \
    ...
    "${RUNPOD_DATASET_REVISION}" "${REMOTE_SELECTION_MOUNT_PATH}" \
    "${RUNPOD_CPU_MAX_RUNTIME_SECONDS}" \
    "${RUNPOD_CPU_PREPARE_RESERVE_SECONDS}" "${RUNPOD_CPU_MAX_API_CALLS}" \
    "${RUNPOD_CPU_EODHD_QPS}" "${RUNPOD_CPU_TAIWAN_QPS}" \
    ...
    "${HF_TOKEN_REFERENCE}" "${EODHD_TOKEN_REFERENCE}" \
    "${TPEX_PROXY_POD_URL}" "${TPEX_PROXY_TOKEN_REFERENCE}" "${FIN_TS_DATASET_PROFILE}" \
    "${FIN_TS_H_START}" \
    "${STAGE1_US_SYMBOLS}" "${STAGE1_US_ETF_SYMBOLS}" "${STAGE1_SYMBOL_LIMIT}" \
    "${STAGE1_DATA_START}" "${STAGE1_DATA_END}")"
