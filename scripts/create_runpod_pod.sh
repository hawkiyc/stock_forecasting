...
WANDB_PROJECT="${WANDB_PROJECT:-stock_forecasting}"
...
POD_ENV_JSON="$(printf \
    ...
    "${RUNPOD_DATASET_REQUEST_SHA256}" "${RUNPOD_DATASET_REVISION}" \
    "${REMOTE_SELECTION_MOUNT_PATH}" \
    "${FIN_TS_DATASET_PROFILE}" "${FIN_TS_H_START}" "${STAGE1_US_SYMBOLS}" \
